...
| 同時接続規模 | ✅ 十分 | Photon無料枠：最大**100CCU**（約16試合同時）|
...
| Edgeリクエスト | 100万回/月 | 静的配信のみ | ✅ 問題なし |
| デプロイ数 | 無制限 | 開発中10〜20回/月 | ✅ 問題なし |
| Function実行時間 | 4時間CPU/月 | 使用しない（静的配信のみ） | ✅ 問題なし |
| カスタムドメイン | 1つ | 1つあれば十分 | ✅ 問題なし |

> ⚠️ **重要**: Vercel Hobby プランは**個人・非商用利用のみ**。商用公開の場合はPro（$20/月）が必要。
> 趣味・ポートフォリオ・テスト目的であれば無料枠で問題なし。

**注意点**: ゲーム本体はすべて静的ファイル（HTML/JS/CSS）として配信するため、Vercelの無料枠で問題なし。WebSocketはVercelでは持てないため、リアルタイム通信は完全にPhotonに委ねる設計が必須。
...
| 制限項目 | 無料枠 | 本ゲームの想定使用量 | 判定 |
|---|---|---|---|
| 同時接続数 (CCU) | **100** | 最大6人/試合 × 16試合 = 96人 | ✅ 余裕あり |
| メッセージ送信レート | 500 msg/秒/room | 6人 × 10msg/秒 = 60msg/秒 | ✅ 大幅余裕 |
| 含有データ転送量 | 約300GB（100CCU × 3GB） | 大幅余裕 | ✅ 問題なし |
| ルーム数 | 制限なし | - | ✅ 問題なし |
| 無料アプリ数 | 1アカウントにつき1つ | 1つあれば十分 | ✅ 問題なし |

**設計判断**:

100CCUという制約から、**同時に最大16試合（96人）まで**が無料枠で対応可能。
個人開発・テストフェーズには非常に十分な規模。
...
| ネットワーク送信 | **1GiB** | ~10MB（APIのみ） | ✅ 余裕あり（静的アセットをCloud Runから配信しない設計が必須）|
...
| Realtime同時接続 | **200接続** | 試合中のリアルタイム更新が必要な場合 | ✅ 余裕あり |
| 不活性化 | **7日間アクセスなしで一時停止** | ⚠️ 要注意 | ⚠️ 対策必要 |

**重要な注意点**: Supabaseの無料プロジェクトは7日間アクセスがないと**自動一時停止**される。
定期的なヘルスチェック（cronジョブ）を設定して対策する。

> Supabaseには Realtime WebSocket機能（200接続/プロジェクト）もあるが、本ゲームではリアルタイム通信をPhotonに集約し、Supabaseは純粋なデータ永続化のみに使う設計が望ましい。
...
### リスク1: Photon 100CCU制限 ⚠️ 低

**問題**: ユーザーが増えると同時に100人以上が接続できない。

**対策**:
- 100CCU = 約16試合同時対応可能。個人開発規模では十分
- 本格公開後に増えたら有料プランへ移行（$95/月〜）
- 代替案: `PartyKit`（Cloudflare Durable Objectsベース）の無料枠も検討

**当初の20CCU制限という情報は古いもので、2025年以降は100CCUに拡張されています。**
